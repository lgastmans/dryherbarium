...
final class StateTable extends PowerGridComponent
{
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ->add('id')
            ->add('name')
            ->add('name_lower', fn (State $model) => strtolower(e($model->name)))
            ->add('created_at')
            ->add('created_at_formatted', fn (State $model) => Carbon::parse($model->created_at)->format('d/m/Y H:i:s'));
    }
    ...
    public function actions(State $row): array
    {
        return [
            Button::make('edit', '<svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                ...
                ->dispatch('openModal', ['edit-state', [ 'state' => $row ]] ),
            ...
            Button::make('destroy', '<svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                ...
                ->openModal('delete-state', ['id' => $row->id]),
        ];
    }
    ...
    /*
    public function actionRules(State $row): array
    {
       return [
