...
final class TalukTable extends PowerGridComponent
{
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ->add('id')
            ->add('name')
            ->add('name_lower', fn (Taluk $model) => strtolower(e($model->name)))
            ->add('created_at')
            ->add('created_at_formatted', fn (Taluk $model) => Carbon::parse($model->created_at)->format('d/m/Y H:i:s'));
    }
    ...
    public function actions(Taluk $row): array
    {
        return [
            Button::make('edit', '<svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                ...
                ->dispatch('openModal', ['edit-taluk', [ 'taluk' => $row ]] ),
            ...
            Button::make('destroy', '<svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                ...
                ->openModal('delete-taluk', ['id' => $row->id]),
        ];
    }
    ...
    /*
    public function actionRules(Taluk $row): array
    {
       return [
